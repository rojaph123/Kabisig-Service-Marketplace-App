    ...
    >
      <SurfaceCard style={{ backgroundColor: theme.colors.surfaceAlt }}>
        <View style={{ flexDirection: "row", gap: 10 }}>
          {[
            { label: "Total earned", value: `PHP ${totals.total.toLocaleString()}`, icon: "wallet-outline" },
            { label: "Paid jobs", value: totals.paidCount.toString(), icon: "checkmark-done-outline" },
            { label: "Pending", value: totals.pendingCount.toString(), icon: "time-outline" }
          ].map((item) => (
        ...
        {visiblePayments.map((payment) => (
          ...
          >
            <SurfaceCard style={{ gap: 12 }}>
              ...
              <Text style={{ color: theme.colors.primaryDark, fontWeight: "800" }}>View earning details</Text>
            </SurfaceCard>
          </Pressable>
        ))}

...
export default function PaymentsTab() {
  ...
  const load = useCallback(async () => {
    ...
    const [bookingDocs, providerDocs] = await Promise.all([
      Promise.all(sorted.map((payment) => bookingService.getBookingById(payment.bookingId))),
      userService.getUsersByIds(sorted.map((payment) => payment.providerId))
    ]);
    setPayments(sorted);
    setBookingsById(
      Object.fromEntries(
    ...
    setProvidersById(Object.fromEntries(providerDocs.map((provider) => [provider.id, provider])));
  ...
  const totals = useMemo(
    () => ({
      total: payments
        .filter((payment) => getEffectivePaymentStatus(payment, bookingsById[payment.bookingId]) === "Paid")
        .reduce((sum, payment) => sum + payment.amount, 0),
      paid: payments.filter((payment) => getEffectivePaymentStatus(payment, bookingsById[payment.bookingId]) === "Paid").length,
      waiting: payments.filter((payment) => getEffectivePaymentStatus(payment, bookingsById[payment.bookingId]) === "Waiting for Completion").length
    }),
  ...
  return (
    <SafeAreaView edges={["top", "left", "right"]} style={{ flex: 1, backgroundColor: theme.colors.background }}>
      <View style={{ paddingHorizontal: theme.spacing.lg, paddingTop: theme.spacing.md, paddingBottom: theme.spacing.sm }}>
        <BackHeader title="Payments" onBack={() => router.back()} />
      </View>
      ...
      <View style={{ paddingHorizontal: theme.spacing.lg, paddingBottom: 10, gap: 10, backgroundColor: theme.colors.background, position: "relative", zIndex: 100, elevation: 20, overflow: "visible" }}>
        <SurfaceCard style={{ backgroundColor: theme.colors.surfaceAlt, gap: 10, padding: 14 }}>
          <Text style={{ color: theme.colors.textMuted, fontSize: 12, fontWeight: "800" }}>Total paid</Text>
          ...
          <View style={{ flexDirection: "row", gap: 8 }}>
            {[
              ...
              { label: "Waiting", value: totals.waiting.toString(), icon: "time-outline" }
            ].map((item) => (
              <View key={item.label} style={{ flex: 1, flexDirection: "row", alignItems: "center", gap: 8, borderRadius: 14, backgroundColor: theme.colors.card, padding: 10 }}>
                ...
                <Text style={{ color: theme.colors.textMuted, fontSize: 12, fontWeight: "800" }}>{item.label}</Text>
              </View>
            ))}
          </View>
        </SurfaceCard>
                ...
                <SurfaceCard style={{ gap: 9, padding: 14 }}>
                  <View style={{ gap: 7 }}>
                    ...
                    <Text style={{ color: theme.colors.primary, marginTop: 4, fontWeight: "800" }} numberOfLines={1}>
                      {providersById[payment.providerId]?.fullName || "Provider"}
                    </Text>
                  ...
                  <View style={{ gap: 3 }}>
                    <Text style={{ color: noPaymentRequired ? theme.colors.textMuted : theme.colors.primary, fontSize: 18, fontWeight: "900" }} numberOfLines={1} adjustsFontSizeToFit minimumFontScale={0.75}>
